...
await fastify.register(fastifyStatic, {
  ...
  prefix: '/static/app/{{APP_NAME}}/frontend/dist/',
});
...
fastify.log.info('✓ {{APP_NAME_TITLE}} plugin registered');
...
const start = async () => {
  try {
    ...
    fastify.log.info('🚀 Starting {{APP_NAME_TITLE}} Server...');
    fastify.log.info(
      {
        port,
        host,
    ...
    fastify.log.info(`✓ API endpoint: http://localhost:${port}/api/{{APP_NAME}}`);
