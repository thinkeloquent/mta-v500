...
class ProxyDispatcherFactory:
    ...
    def _resolve_proxy_url(self, environment: Optional[AppEnv] = None) -> Optional[str]:
        """
        ...
        if self._config.proxy_urls:
            return getattr(self._config.proxy_urls, env, None)
        return None

    def get_proxy_dispatcher(
        self,
        environment: Optional[AppEnv] = None,
        ...
        # TLS disabled only in DEV unless overridden
        verify_ssl = not disable_tls if disable_tls is not None else (env != Environment.DEV)
