"""
Shared async HTTP client for Figma API with rate limiting and retry logic.
"""
import asyncio
import ssl
import time
...
class FigmaHttpClient:
    """
    Shared async HTTP client for Figma API.
    ...
    def __init__(
        ...
        """
        Initialize Figma HTTP client.

        Args:
            token: Figma API token (from process.env['FIGMA_TOKEN'])
    ...
    async def __aenter__(self) -> "FigmaHttpClient":
    ...
    def _get_client(self) -> httpx.AsyncClient:
        ...
        if self._client is None:
            raise RuntimeError(
                "Client not initialized. Use 'async with FigmaHttpClient(...) as client:'"
            )
        return self._client
