 ...
 */
function formatRoutes(routes: string, options: Required<RouteLoggerOptions>): string {
...
function flattenRoutesTree(routesString: string): string {
  ...
  for (const line of lines) {
    ...
    if (methodMatch) {
      ...
      // Build full path
      const fullPath =
        '/' + pathStack.filter(Boolean).join('').replace(/\/+/g, '/').replace(/^\//, '');
 ...
 */
async function logToFile(formattedRoutes: string, outputPath: string, logger: any): Promise<void> {
  try {
 ...
 */
function logToFastifyLogger(routesString: string, routeCount: number, logger: any): void {
 ...
 */
const routeLoggerPlugin: FastifyPluginAsync<RouteLoggerOptions> = async (fastify, options) => {
  ...
  fastify.addHook('onReady', async () => {
    ...
    try {
      ...
      fastify.log.debug(
        { hook: 'onReady', plugin: 'core-route-logger', duration: hookDuration },
