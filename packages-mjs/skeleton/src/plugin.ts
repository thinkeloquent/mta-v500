...
export default async (fastify: FastifyInstance) => {
  ...
  await fastify.register(headerRequestEnhancements, {
    ...
    corsAllowLocalhost: process.env.NODE_ENV !== 'production',
  ...
  fastify.get<{ Params: { name: string } }>('/sys/status/apps/:name', async (request, reply) => {
    ...
    return {
      ...
      error: app.error
        ? {
            ...
            stack: process.env.NODE_ENV === 'development' ? app.error.stack : undefined,
          }
        : undefined,
    };
  });
