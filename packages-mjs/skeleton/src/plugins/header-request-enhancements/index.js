...
import { createCorsPolicyEngine } from '../../lib/cors-policy-engine.js';
 ...
 * ```
 */
const headerRequestEnhancementsPlugin = async (fastify, options) => {
    ...
    if (pluginConfig.cors) {
        ...
        if (useCorsEngine) {
            ...
            if (config.corsEngine) {
                ...
                if (!validation.valid) {
                    fastify.log.error({ errors: validation.errors }, 'CORS configuration validation failed');
                    throw new Error(`CORS configuration invalid: ${validation.errors.join(', ')}`);
                }
                if (validation.warnings.length > 0) {
                    fastify.log.warn({ warnings: validation.warnings }, 'CORS configuration warnings');
                }
            ...
            const corsOptions = policyEngine.getFastifyCorsOptions(config.appName);
            ...
            fastify.log.warn({ mode: corsMode }, 'Using legacy CORS configuration. Consider migrating to useCorsEngine for enterprise features.');
            // Add manual CORS headers via onSend hook for all responses
