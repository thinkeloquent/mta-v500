 ...
 */
export function loadCorsConfig(options = {}) {
    const { configPath, environment = process.env.NODE_ENV || 'development', logger, monorepoRoot = process.env.MONOREPO_ROOT, } = options;
    const envOverrides = [];
    let config;
    let source;
    ...
    if (resolvedPath && existsSync(resolvedPath)) {
        try {
            ...
            logger?.error({ error: error.message, path: resolvedPath }, 'Failed to load CORS config file, using defaults');
            config = structuredClone(DEFAULT_CONFIG);
            source = 'default (file load failed)';
        }
    }
    else {
        logger?.info({ searchedPath: resolvedPath }, 'CORS config file not found, using defaults');
        config = structuredClone(DEFAULT_CONFIG);
        source = 'default';
    }
 ...
 */
function resolveConfigPath(configPath, monorepoRoot) {
 ...
 */
function applyEnvironmentOverrides(config, environment, envOverrides, logger) {
    ...
    if (corsAllowOrigins) {
        ...
        if (origins.length > 0) {
            ...
            logger?.info({ origins: origins.map((o) => o.value || o.pattern), environment }, 'Applied CORS_ALLOW_ORIGINS override');
        }
    }
    ...
    if (corsLogLevel && ['debug', 'info', 'warn', 'error'].includes(corsLogLevel)) {
        config.global.logging.level = corsLogLevel;
 ...
 */
export function validateCorsConfig(config, environment, logger) {
    const errors = [];
    const warnings = [];
    // Check version
    if (!config.version) {
    ...
    if (environment === 'production') {
        if (!envConfig?.origins || envConfig.origins.length === 0) {
            ...
            if (!hasEnvOverride) {
                errors.push('Production requires explicit origins. Set CORS_ALLOW_ORIGINS or configure in cors.json');
            }
        }
    ...
    if (errors.length > 0 || warnings.length > 0) {
        logger?.info({ errors, warnings, environment }, 'CORS configuration validation results');
    }
    return {
 ...
 */
export function getCorsConfigSummary(config, environment) {
    const envConfig = config.environments[environment];
    const lines = [
...
//# sourceMappingURL=cors-config-loader.js.map