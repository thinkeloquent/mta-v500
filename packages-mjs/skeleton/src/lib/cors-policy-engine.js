 ...
 */
export function compileOriginPattern(pattern, namedPatterns) {
    switch (pattern.type) {
        case 'exact':
            if (!pattern.value) {
            ...
            return {
                ...
                test: (origin) => origin === pattern.value,
...
export class CorsPolicyEngine {
    ...
    logger;
    constructor(config, environment = 'development', logger) {
        this.config = config;
    ...
    validateConfig() {
        ...
        if (!envConfig) {
            this.logger?.warn({ environment: this.environment, available: Object.keys(this.config.environments) }, 'Environment not found in CORS config, using development defaults');
        }
        ...
        if (envConfig?.origins) {
            for (const pattern of envConfig.origins) {
                try {
                    ...
                    throw new Error(`Invalid origin pattern in ${this.environment}: ${error.message}`);
                }
            }
        }
        ...
        if (this.environment === 'production' &&
            ...
            throw new Error('Production requires explicit origins but none configured. ' +
     ...
     */
    getAllowedOriginHeader(origin, appName) {
        const decision = this.checkOrigin(origin, appName);
 ...
 */
export function createCorsPolicyEngine(config, environment, logger) {
    const env = environment || process.env.NODE_ENV || 'development';
    return new CorsPolicyEngine(config, env, logger);
}
export default CorsPolicyEngine;
//# sourceMappingURL=cors-policy-engine.js.map