    ...
    async loadApps(appConfigs, databaseUrl) {
        ...
        for (const appConfig of appConfigs) {
            ...
            try {
                ...
                if (backendExists) {
                    ...
                    this.fastify.log.info({ appName: appConfig.name }, `✓ Loaded backend plugin for: ${appConfig.name}`);
                }
                ...
                if (frontendExists) {
                    ...
                    this.fastify.log.info({ appName: appConfig.name }, `✓ Loaded frontend plugin for: ${appConfig.name}`);
                }
                ...
                const appDatabaseUrl = process.env[`${appConfig.name.toUpperCase()}_DATABASE_URL`] || databaseUrl;
        ...
        for (const appName of apps) {
            ...
            if (app && app.status === 'loaded') {
                ...
                this.fastify.log.info({ appName }, `✓ Registered explicit route: GET /sys/status/apps/${appName}`);
            }
        }
