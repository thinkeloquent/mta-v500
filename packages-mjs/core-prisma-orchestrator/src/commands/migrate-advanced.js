 ...
 */
export async function migrateDiffCommand(config, options = {}) {
 ...
 */
export async function migrateResolveCommand(config, migration, options = {}) {
 ...
 */
export async function migrateBaselineCommand(config, migration, options = {}) {
    ...
    for (const app of apps) {
        logger.section(app.name);
        // Create migration
        const createResult = await executePrisma(app, 'migrate dev', ['--name', migration, '--create-only'], { silent: false });
...
//# sourceMappingURL=migrate-advanced.js.map