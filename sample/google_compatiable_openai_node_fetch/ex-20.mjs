...
function formatOutput(originalCode, changes, response, requestOptions = {}) {
  console.log(`\n${SEPARATOR}`);
  console.log("  GEMINI REACT COMPONENT MODIFIER");
 ...
 */
function defineReactModifierSchema(componentName, changes) {
  const changesText = changes
    .map((change, i) => `  ${i + 1}. ${change}`)
    .join("\n");

  const systemPrompt = `You are a React component modifier. You will receive a React component and a list of changes to apply.
...
/**
 * Performs a chat completion request for React component modification.
 * @param {Object} clientConfig - Client configuration from createClientConfig()
 * @param {Array} messages - Array of message objects with role and content
 * @param {Object} modifierSchema - Schema from defineReactModifierSchema()
 * @param {Object} options - Request options
 * @returns {Promise<Object>} Chat completion response with parsed modification
 */
async function chatCompletionModifyComponent(
  clientConfig,
  messages,
  modifierSchema,
  options = {}
  ...
  if (!modifierSchema) {
    throw new Error("modifierSchema is required for component modification");
  }
  ...
  for (const msg of messages) {
    if (msg.role === "system") {
      messagesWithSchema.push({
        role: "system",
        content: `${modifierSchema.systemPrompt}\n\nAdditional instructions: ${msg.content}`,
      });
...
async function main() {
  logProgress("INIT", "Starting Gemini React Component Modifier...");
...
export default function BasicCard() {
  ...
  try {
    ...
    const modifierSchema = defineReactModifierSchema("BasicCard", changes);
    logProgress("SCHEMA", "Created modifier schema");

    // Build user prompt with template (RAG-style)
    const userPrompt = `Please modify the following React component according to the specified changes.
...
Apply all the changes listed in the system prompt and return the modified component.`;
    ...
    const response = await chatCompletionModifyComponent(
      clientConfig,
      [
        {
          role: "system",
          content: "Apply changes precisely while maintaining code quality.",
        },
    ...
    logProgress(
      "COMPLETE",
      "React component modification completed successfully"
    );
  } catch (error) {
