...
// Parse streaming response based on mode
function parseStreamChunk(chunk: string, mode: StreamMode): string {
  if (mode === 'text-stream') {
  ...
  for (const line of lines) {
    ...
    if (line.startsWith('0:')) {
      try {
        ...
        // If JSON parse fails, try to extract content directly
        result += line.slice(2);
      }
    }
    // Data/init message: 2:[...] - skip (initialization data)
    // Error message: e:{...} - could handle separately if needed
  }
...
function App() {
  ...
  const [streamMode, setStreamMode] = useState<StreamMode>('text-stream');
  ...
  const getEndpoint = (mode: StreamMode, format: InputFormat): string => {
    // Use /api/apps/ai-sdk-chat/chat when served via MTA orchestrator
    const basePath = '/api/apps/ai-sdk-chat/chat';
    const suffix = format === 'prompt' ? '-prompt' : '';
    switch (mode) {
      case 'data-stream':
        return `${basePath}/stream-protocol${suffix}`;
      case 'text-stream':
        return `${basePath}/stream-text${suffix}`;
      case 'custom-stream':
        return `${basePath}/stream-custom${suffix}`;
    }
  };
      ...
      try {
        ...
        if (reader) {
          while (true) {
            ...
            const parsed = parseStreamChunk(chunk, streamMode);
        ...
        setPromptResponse(`Error: ${error instanceof Error ? error.message : 'Failed to get response from server.'}`);
      } finally {
      ...
      <header className="bg-white shadow-sm border-b border-gray-200">
        <div className="max-w-4xl mx-auto px-4 py-4">
          ...
          <p className="text-sm text-gray-600 mt-1">
            Demonstrating OpenAI-compatible streaming with FastAPI
          </p>
        </div>
      </header>
