...
@router.get(
    ...
    summary="List available S3 buckets (lightweight)",
    description="Retrieve a lightweight list of bucket names and creation dates. This is faster than the full bucket list as it doesn't fetch region, object counts, etc.",
    responses={
        200: {
...
async def list_available_buckets(s3_service: S3Service = Depends(get_s3_service)) -> Dict[str, Any]:
    """
    List available S3 buckets with minimal metadata (name and creation date only).
    This is optimized for fast initial loading.

    Returns:
        Dict with success status and list of buckets (name and creationDate only)
    """
    try:
        buckets = await s3_service.list_available_buckets()

        # Log bucket names for debugging
        bucket_names = [b.get("name") for b in buckets if b.get("name")]
        logger.info(f"Returning {len(buckets)} available buckets: {bucket_names}")

        return {"success": True, "buckets": buckets}
    except Exception as e:
        logger.error(f"Failed to list available buckets: {e}")
        raise HTTPException(
...
async def list_buckets(s3_service: S3Service = Depends(get_s3_service)) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Returning {len(buckets)} buckets to client")
        logger.debug(f"Response structure: {{'success': True, 'buckets': {buckets}}}")

        # Validate response structure
        for idx, bucket in enumerate(buckets):
            if not isinstance(bucket, dict):
                logger.error(f"Bucket at index {idx} is not a dict: {type(bucket)}")
            elif not bucket.get("name"):
                logger.error(f"Bucket at index {idx} missing 'name' field: {bucket}")
