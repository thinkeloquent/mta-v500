...
from ..config.aws import get_aws_clients
...
class S3Service:
                    ...
                    buckets_with_details.append(
                        {
                            ...
                            "region": region,
                            "objectCount": objects_response.get("KeyCount", 0),
                            "hasObjects": bool(objects_response.get("Contents")),
                        }
                    )
    ...
    async def delete_bucket(self, bucket_name: str) -> Dict[str, Any]:
        ...
        Raises:
            ValueError: If bucket is not empty or has access points
        """
        try:
            ...
            if objects_response.get("Contents"):
                raise ValueError("Cannot delete bucket that contains objects")
            ...
            try:
                ...
                if "access points attached" in error_msg.lower():
                    account_id = settings.aws_account_id

                    if not account_id:
                        raise ValueError(
                            "Bucket has access points attached and cannot be deleted. "
                            "Please set AWS_ACCOUNT_ID environment variable to enable "
                    ...
                    try:
                        ...
                        if access_points:
                            access_point_names = [ap["Name"] for ap in access_points]
                            error = ValueError(
                                f"Bucket has {len(access_points)} access point(s) attached: "
                                f"{', '.join(access_point_names)}. These must be deleted "
                                "before the bucket can be removed."
                            )
                            error.code = "BUCKET_HAS_ACCESS_POINTS"  # type: ignore
                            error.access_points = access_points  # type: ignore
                            raise error
                    except ValueError as ap_error:
                        if hasattr(ap_error, "code") and ap_error.code == "BUCKET_HAS_ACCESS_POINTS":  # type: ignore
                            raise
                        raise ValueError(f"Failed to check access points: {ap_error}") from ap_error

                raise delete_error
        except ClientError as e:
            error_msg = f"Failed to delete bucket: {e}"
            logger.error(error_msg)
            raise Exception(error_msg) from e
